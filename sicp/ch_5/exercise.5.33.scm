...
;;; Supporting functions
  ...
  (define (code-for-next-arg operand-codes)
    (preserving
     '(argl)
     (car operand-codes)
  ...
  (define (construct-arglist operand-codes) ; FIX: I don't understand this
    (let ((operand-codes (reverse operand-codes)))
      (if (null? operand-codes)
          ...
          (let ((code-to-get-last-arg (code-to-get-last-arg operand-codes)))
            (if (null? (cdr operand-codes))
                code-to-get-last-arg
                (preserving '(env)
                            code-to-get-last-arg
  ...
  (define (new-label-number)
    ;; TODO: remove this and use a dictionary to make numbers per label
