...
(provide smalltalk/compile)

(define (smalltalk/compile stx)
  (printf "~a~n~n" (syntax->datum stx))
  ...
  (define (r/list stx)
    (map smalltalk/compile (syntax->list stx)))

  (syntax-parse stx
    [({~datum parse} (~seq class-def "!") ...+ (~optional "!"))
     (r/list #'(class-def ...))]

    [({~datum classDefinition} hdr (~seq meth "!") ...)
     ;; (r/list #'(meth ...))
     42
     ]

    [({~datum classDefinition} (~seq static) ...+)
     (r/list #'(static ...))]

    [({~datum staticStatement} c-name keyword str (~optional "."))
     44]

    [({~datum staticStatement} k1 "#" c-name k2 s2 k3 s3 k4 s4 k5 i-or-s (~optional "."))
     44]

    ;; [({~datum staticStatement} (~seq subs) ...)
    ;;  45]

    ;; [({~datum classHeader} "!" (~seq subs) ... "!")
    ;;  43]
    ;; [({~datum method} (~seq subs) ...)
    ;;  43]

    ;; [({~datum parse} ({~datum classDefinition} (~rest statics))) #(syntax->datum #'statics)]
    ;; (classDefinition (staticStatement (className (identifier "nil")) (keyword "subclass:") "#" (className (identifier "Object")) (keyword "instanceVariableNames:") (string "''") (keyword "classVariableNames:") (string "''") (keyword "poolDictionaries")))
    )
