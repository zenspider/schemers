...
;; a logarithmic number of steps. Recall the transformation of the
;; state variables a and b in the `fib-iter' process of section *Note

;; 1-2-2: a <- a + b and b <- a. Call this transformation T, and
;; observe that applying T over and over again n times, starting with
;; 1 and 0, produces the pair _Fib_(n + 1) and _Fib_(n). In other
...
;; according to a <- bq + aq + ap and b <- bp + aq. Show that if we
;; apply such a transformation T_(pq) twice, the effect is the same as
;; using a single transformation T_(p'q') of the same form, and
;; compute p' and q' in terms of p and q. This gives us an explicit
;; way to square these transformations, and thus we can compute T^n
;; using successive squaring, as in the `fast-expt' procedure. Put
;; this all together to complete the following procedure, which runs
;; in a logarithmic number of steps:(5)
